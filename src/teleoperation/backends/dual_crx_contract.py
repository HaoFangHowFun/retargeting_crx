"""ROS-free position mapping for the dual_crx_ros2 command contract."""
...
BIMANUAL_CRX_ARM_NAMES = tuple(
    f"{side}_J{i}" for side in ("left", "right") for i in range(1, 7)
)
BIMANUAL_CRX_HAND_NAMES = tuple(
    f"{side}_leap_joint_{i}" for side in ("left", "right") for i in range(16)
)
BIMANUAL_CRX_NAMES = tuple(
    chain.from_iterable(
...
def to_dual_crx_names(names: Sequence[str] | None = None) -> tuple[str, ...]:
    """Return canonical dual-crx names for one CRX+LEAP position vector."""
...
def to_bimanual_crx_positions(
    ...
    if values.shape != (44,):
        raise ValueError(
            f"Bimanual CRX+LEAP qpos must have shape (44,), got {values.shape}."
        )
    if not np.isfinite(values).all():
...
def join_bimanual_crx_positions(
    ...
    """Join two 22-DoF robot vectors in canonical bimanual order."""
...
def to_dual_crx_command(
    ...
    *,
    ...
    """Return validated names and positions for ``TeleopCommand``."""
